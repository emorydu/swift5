# Extension
1. 给已定义的类型和protocol 新增属性（不能是存储属性，因为影响建立这个类型本身，导致启动方式不可用、类型大小改变）、方法或protocol
```swift
extension Int {
...
## 总结
### Extension建立方法
1. 通过关键字extension宣告，后面加上类型或是protocol
2. 在宣告内容中可以直接存取内部属性和方法
3. 指定protocol中的generic类型可以使用where T: Protocol和 where T == Type
### Extension可以新增
1. 计算属性和方法（只有实例的存储属性不能新增）
2. 启动方法
3. 下标subscript
4. 类型
5. protocol
### 无论是在同一个文件中或是跨文件都可以帮助归纳整理code
